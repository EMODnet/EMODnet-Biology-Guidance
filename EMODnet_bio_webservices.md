...
The EMODnet Biology data are available as Web Services:

1. **Occurrence data (species observations) as Web Feature Services (WFS)**

   - The EMODnet Biology data is available in four standard file formats. This is specified at the end of the WFS request:

     - CSV  `&outputformat=csv` 
     - JSON `&outputformat=application/json`
     - Google KML `&outputformat=application/vnd.google-earth.kml+xml`
     - ESRI Shapefile `&outputformat=SHAPE-ZIP`
     
   - There are [three format options](https://www.emodnet-biology.eu/emodnet-data-format) offered by EMODnet Biology which can be retrieved:
     
     - **Basic Occurrence Data**
     
       Adding the following sentence in your GET request will return the basic occurrence data: `&typeName=Dataportal:eurobis-obisenv_basic`
     
       For example, this request will return the first 50 basic occurrences from the dataset *Monitoring of birds in the Voordelta* ([4569](http://www.emodnet-biology.eu/data-catalog?module=dataset&dasid=4659)) as csv:
